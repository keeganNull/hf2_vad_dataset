...
def samples_extraction(dataset_root, dataset_name, mode, all_bboxes, save_dir):
    ...
    print(chunked_samples["sample_id"])
    if len(chunked_samples["sample_id"]) != 0:
        ...
        chunked_samples["pred_frame"] = np.array(chunked_samples["pred_frame"])
        joblib.dump(chunked_samples, os.path.join(save_dir, "chunked_samples_%02d.pkl" % chunk_id))
        print(os.path.join(save_dir, "chunked_samples_%02d.pkl" % chunk_id))
        print("Chunk %d file saved!" % chunk_id)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--proj_root", type=str, default="/data1/hzj_1", help='project root path')
    ...
    all_bboxes = np.load(
        os.path.join(args.proj_root,  args.dataset_name, '%s_bboxes_%s.npy' % (args.dataset_name, args.mode)),
        allow_pickle=True
    )
    if args.mode == "train":
        save_dir = os.path.join(args.proj_root,  args.dataset_name, "training", "chunked_samples")
    else:
        save_dir = os.path.join(args.proj_root,  args.dataset_name, "testing", "chunked_samples")

    samples_extraction(
        dataset_root=os.path.join(args.proj_root),
